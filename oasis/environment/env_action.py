...
class ExternalAction:
    r"""Represents an action delegated to an external HTTP Agent.

    When env.step() encounters this action, it will HTTP-call the external
    agent's endpoint with the current feed, and the external agent returns
    the action(s) to execute.

    Args:
        endpoint: The HTTP URL of the external agent (e.g. "http://localhost:5001/act").
        timeout: Timeout in seconds for the HTTP call.
        extra_context: Optional extra data to send along with the feed.
    """
    endpoint: str
    timeout: float = 30.0
